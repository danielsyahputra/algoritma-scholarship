...
model_ps_no <- lm(formula = Profit ~ Sales, data = copiers_no)
model_ps_no
```
...
abline(model_ps_no, col = "red")
abline(model_ps, col = "blue")
```

Formula model outlier:
...
-   Apabilla nilai R\^2 pada model yang memiliki outlier lebih baik
    tanpa outlier maka pilih model yang menggunakan outlier.
...
summary(model_ineq)
model_ineq$coefficients
```

Insight

-   Persamaan regresi: `Inequality = 386.03 - 0.36551 * gdp`
...
Variabel yang berpengaruh terhadap inequality adalah:
...
-   harus cek range data untuk menentukan tingkat toleransi error
...
```{r}
MSE(y_pred = model_ineq_all$fitted.values, y_true = crime$inequality)
```
...
-   Tidak ada fitur yang berkorelasi negatif

-   Nilai korelasi tertinggi adalah 0.8 yaitu korelasi
    `sqft_living ~ bathrooms` , `sqft_living ~ grade`

    buat model dengan keseluruhan variable

```{r}
model_house <- lm(formula = price ~ ., data = house)
```
...
-   variabel prediktor yang signifikan?
    ...
    Setiap predictor merupakan predictor yang signifikan karena memiliki
    level signifikan `***`

```{=html}
<!-- -->
```
-   goodness of fit (adj r-quared): `64%`

```{r}
summary(model_house)$adj.r.squared * 100
```
...
```{r}
RMSE(y_pred = model_house$fitted.values, y_true = house$price)
```
...
Nilai `price` pada data `house` ada pada skala jutaan (10\^6) dan nilai
`RMSE = 218864.1` masih pada skala ribuan. Jadi menurut saya modelnya
bisa dikatakan tidak terlalu buruk.

```{r}
MAE(y_pred = model_house$fitted.values, y_true = house$price)
```
