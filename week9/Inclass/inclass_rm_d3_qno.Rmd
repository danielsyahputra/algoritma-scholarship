...
```{r}
model_crime_2 <- lm(formula = inequality ~ ., data = crime2)
summary(model_crime_2)
```
...
model_ineq_non <- lm(formula = inequality ~ 1, data = crime)
...
```{r}
#model backward
model_backward <- step(object = model_ineq_all, direction="backward")
```
...
4.  Mengulangi step 2 dan 3 hingga nilai AIC terbaol dihasilkan ketika
...
```{r}
#model both
model_both <- step(object = model_ineq_non, scope = list(upper=model_ineq_all), data=crime2, direction="both")
```
...
Model terbaik berdasarkan adjusted r-squared terbesar adalah
`model_backward`
...
1.  Handle outliers, tentukan apakah dia memiliki high influence atau
    tidak

2.  Melakukan feature selection dengan step-wise regression

3.  Menggunakan variabel prediktor yang signifikan

Prediksi data crime berdasarkan model backward

```{r}
hasil <- predict(object = model_backward, newdata = crime)
crime$hasil <- hasil
head(crime)
```
...
Confidence level yang biasa diguanakan = 90%, 95%, 99%

Mencari hasil prediksi dengan confidence interval 95%

```{r}
# cek confidence interval 95%
head(predict(object = model_backward, newdata = crime, interval = "prediction", level = 0.95))
```

```{r}
# cek confidence interval 100%
head(predict(object = model_backward, newdata = crime, interval = "prediction", level = 1))
```

```{r}
# cek confidence interval 0%
head(predict(object = model_backward, newdata = crime, interval = "prediction", level = 0))
```
...
Evaluasi model_ineq_all, model_ineq2, model_ineq dan model hasil
stepwise
...
estimate/koefisien prediktor yan bias (terlalu sempit atau terlalu
lebar).
