...
```{r}
# yang perlu di cek keseimbangannya adalah kelas target
...
data yang dipakai untuk belajar 80%

depan : gambar belakng jawwab

data yang digunakan untuk mengevaluasi proses pembelajaran (20%) dari 20
gambar, berapa banyak sih yang ditebak dengan benar?

50 : mobil 50 : motor

cross validation itu memisahkan data menjadi 2 bagian yaitu data train
dan data test data train : akan digunakan pada saat proses pembuatan
model 80% data test : akan digunakan pada saaat mengevaluasi model
(predict) 20%

data full : - 0 : 75% - 1 : 25%

apakah datanya diseimbangkan dulu baru di split, atau datanya di split
dulu kemudian data train diseimbangkan?

data test harus seoriginal mungkin, karena data test harus menggambarkan
kondisi data di lapangan. split data terlebih dahulu, kemudian data
train di seimbangkan

data full : - 0 : 75% - 1 : 25%

data test : - 0 : 75% - 1 : 25%

data train (sudah di seimbangkan) : - 0 : 50% - 1 : 50%
...
1,556 data : 80% nya akan digunakan sebagai data train, sisanya 20% akan
digunakan sebagai data testing. proses pemmgabilan data dilakukan secara
acak
...
# set.seed digunakan untuk mengunci sifat random dari fungsi sample()
set.seed(417) # anggap saja angka ini sebagai password

index_loan <- sample(x = nrow(loans_clean) , size = nrow(loans_clean)*0.8) 
loan_train <- loans_clean[index_loan , ]
loan_test <- loans_clean[-index_loan, ] # tanda - berarti negasi
```
...
```{r}
summary(loan_train$home_ownership)
```
...
> int_rate: Setiap kenaikan int_rate sebesar 1 satuan maka kecenderungan
> nasabah untuk gagal bayar naik sebesar 1.01 kali dengan catatan
> variabel lainnya konstan

Lakukan feature selection menggunakan stepwise

menghilangkan predictor satu persatu sampai didapati nilai AIC terrendah

```{r}
# logistic regression juga dapat menggunakan step
creditrisk2 <- step(object = creditrisk, direction="backward", trace = 0)
summary(creditrisk2)
```
...
mas saya bingung cari tau yang peluangnya lebih dari 0.5 itu yang "1"
atau "0"

```{r}
levels(loan_test$not_paid)
```

Label: "0" x "1" Probability: 0 0.5 1

```{r}
# ubah probability menjadi label 
loan_pred_label <- as.factor(ifelse(loan_pred > 0.5, "1", "0")) 
```

berapa banyak dari data test yang diprediksi 1?

```{r}
table(loan_pred_label)
```

Target berupa numeric : error = actual - predict (RMSE, MSE) Target
berupa categoric: label pada hasil prediksi dengan actual apakah sama
atau beda
...
TRUE : diprediksi dengan benar FALSE : diprediski dengan salah

Positive: kelas yang menjadi fokus amantan Negative: yang bukan fokus
amatan
...
```{r}
(94+97)/nrow(loan_test)
```

100 Data 90 No 10 Yes
...
Cara memilih recall/ precision sebagai metrics?

1\. Definisikan dahulu kelas positive-nya (covid)

2\. Definisisikan resiko dari kesalahan prediksi

FN :

actual: Covid -> predict :sehat

\- persebaran covid akan lebih cepat diluar sana

\- ...

\- ...

FP : actual: Sehat -> predict : Covid

\- Beban mental, karena dikucilkan warga

\- ...

\- ...

3\. Kesalahan mana yang bisa kita toleransi (ambil yang terbaik dari
yang terburuk)

\- Kita gunakan \*recall\* ketika FP ditoleransi. prediksi aja semuanya
covid (kasarnya)

\- kita gunakan \*precision\* ketika FN ditoleransi.
...
-   Metrics = Recall, karena kalau penyakitnya makin dibiarin makin
    bahaya.
...
-   Kelas positif = spam
-   Metrics = Precision, karena kita ga mau email penting tapi malah
    masuk spam.
...
Adakah kolom yang perlu dibuang dan tipe data yang harus diubah? -
Membuang kolom id - Mengubah kolom diagnosis menjadi factor dan
mengganti label nya menjadi "Benign" dan "Malignant"
...
-   **cek proporsi kelas**: ingin yang seimbang
-   **cek range nilai tiap variable prediktor**: range harus sama karena
    knn mengklasifikasikan berdasarkan jarak. Jika terdapat range nilai
    yang berbeda jauh antar variabel, maka variable tersebut akan sangat
    mempengaruhi hasil klasifikasi dan mengabaikan variable yang lain.
    Oleh karena itu perlu dilakukan scalling.
...
Range tiap variabel berbeda sehingga perlu dilakukan feature rescaling
di tahap *data pre-processing*, bisa menggunakan *min-max normalization*
atau *z-score standarization*.
...
Note: - Min-Max/normalize range 0 s/d 1 - z-score range -inf s/d +inf

### Cross Validation

```{r}
set.seed(100)
index_wbcd <- sample(nrow(wbcd_clean), nrow(wbcd_clean)*0.8)
wbcd_train <- wbcd_clean[index_wbcd,]
wbcd_test <- wbcd_clean[-index_wbcd,]
```

Sebelum melakukan scaling kita perlu misahkan predictor dan target
...
Lakukan scaling untuk data train dan dan data tes
...
Pemilihan nilai k dari akar observasi (data train)
...
lakukan pemodelan menggunakan fungsi knn() dari package class

```{r}
library(class)
wbcd_pred <- knn(train = train_x_scaled, test = test_x_scaled, cl = train_y$diagnosis, k = 21) 
```
...
confusionMatrix(data = wbcd_pred, reference = test_y$diagnosis, positive = "Malignant")
```
...
\- dia tidak membuat model, tidak menampilkan peluang, dan tidak bisa
diinterpretasi
