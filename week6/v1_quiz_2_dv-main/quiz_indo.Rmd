...
```{r}
# your code here
loan_us <- loan[loan$country == "United States", ]
head(loan_us)
```
...
```{r}
# your code here
loan_manufactur <- loan[loan$sector == "Manufacturing", ]
head(loan_manufactur)
```
...
```{r}
# your code here
plot(loan_manufactur$loan_amount, loan_manufactur$lender_count)

```

```{r}
cor(loan_manufactur$loan_amount, loan_manufactur$lender_count)
```
...
3.  Manakah dari pernyataan berikut yang **BENAR** sesuai dengan scatter plot yang sudah dibuat?
    ...
    -   [x] Terdapat pinjaman yang memiliki jumlah pinjaman yang besar dengan jumlah pemberi pinjaman yang sedikit
...
library(lubridate)
# your code here
loan_philippine <- loan[loan$country == "Philippines", ]
loan_philippine$posted_time <- ymd_hms(loan_philippine$posted_time)
```

```{r}
head(loan_philippine)
```
...
min(loan_philippine$posted_time)
```
...
```{r}
# your code here
loan_philippine$funding_duration <- difftime(loan_philippine$funded_time, loan_philippine$posted_time, units = "hours")
loan_philippine
```
...
```{r}
# your code here
loan_philippine$month <- month(loan_philippine$posted_time, label = T, abbr = F)
head(loan_philippine$month)
```
...
```{r}
# your code here
loan_agg <- aggregate(funding_duration ~ month + repayment_interval, data = loan_philippine, FUN=mean)
...
```{r}
ggplot(loan_agg, aes(x = month, y = funding_duration, color = c("red", "blue", "green"), group = repayment_interval))+
  geom_line()+
  geom_point()+
...
**Referensi Opsi Bahasa Inggris:** - \[ \] Monthly repayment interval has almost the same funding duration with Irregular repayment interval in August - \[ \] Bullet repayment interval has longer funding duration than Irregular repayment interval in June - \[ \] Monthly repayment interval never funded faster than Irregular repayment interval
