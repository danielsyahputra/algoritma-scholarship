...
ggplot(data = publish_category, mapping = aes(x = freq, y = reorder(kategori, freq))) +
  ...
  geom_text(aes(label = freq, group = rentang),
...
`Education`,dan `Science and Technology` dari hari ke hari.
...
```{r}
# Please run the code down below
categories <- c("Comedy", "Education", "Science and Technology")
vids.ces <- vids[vids$category_id %in% categories, ]
head(vids.ces)
```

```{r}
unique(vids.ces$category_id)
```
...
```{r}
# Please run the code down below
vids.ces <- as.data.frame(table(vids.ces$category_id, vids.ces$trending_date))
colnames(vids.ces) <- c("Kategori", "TrendingDate", "Freq")
vids.ces
```
...
```{r}
# Please type your code down below
ggplot(data = vids.ces, mapping = aes(x = ymd(TrendingDate), y = Freq)) +
...
ggplot(data = vids.ces, mapping = aes(x = ymd(TrendingDate), y = Freq)) +
  ...
  labs(title = "Frequency of Category Vs Trending Date",
       subtitle = "Youtube Trending Data",
       x = "Trending Date",
       y = 'Frequency') + 
  theme_clean() +
...
-   Apakah barplot bisa untuk visualisasi dengan additional kolom selain
    default dari plot
...
```{r}
categories <- c("Gaming", "Entertainment", "Music")
filtered_vids <- vids[(vids$category_id %in% categories & vids$trending_date >= "2018-01-01"), ]
filtered_vids$category_id <- droplevels(filtered_vids$category_id)
trending_categories <- as.data.frame(table(filtered_vids$category_id, filtered_vids$trending_date))
colnames(trending_categories) <- c("Kategori", "TrendingDate", "Freq")
head(trending_categories)
```

```{r}
ggplot(data = trending_categories, mapping = aes(x = ymd(TrendingDate), y = Freq)) +
  geom_line(aes(color = Kategori)) +
  geom_point(shape=21, color="black", fill="#69b3a2", size=2) + 
  scale_x_date(date_breaks = "2 day", date_labels = "%m-%d") +
  scale_y_continuous(breaks = seq(0, 70, 5)) + 
  labs(title = "Trending Konten: Entertainment Vs Gaming Vs Music (2018)",
       subtitle = "Source: Youtube Trending Data",
       x = "Tanggal Trending",
       y = 'Frekuensi') + 
  theme_clean() +
  theme(plot.title = element_text(hjust = 0.5, face="bold"),
...
-   Channel dengan kategori Entertainment yang paling diminati dibanding
    2 kategori lainnya.

-   Perubahan frekuensi dari hari ke hari pada kategori Gaming cukup
    stabil dibandingkan 2 kategori lainnya.
