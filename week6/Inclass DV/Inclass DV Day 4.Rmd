...
-   parameter `label =` = untuk mengkestrak tanggal dalam bentuk nama
    bulan
-   parameter `abbr` = untuk mengekstrk tanggal dalam bentuk singaktan
    atau tidak
...
-   `X` adalah parameter u/ mendifine kolom apa yang ingin kita
    transformasi
-   `FUN` adalah parameter untuk memasukan nama fungsi yang sudah kita
    buat
...
-   `Libray(lubridate)`
    ...
    -   berisikan fungsi-fungsi untuk melakukan transformasi data
        menjadi datetime
    -   berisikan fungsi-fungsi untuk mengkestrak data tangal ke kolom
        baru

-   Fungsi If Else

    -   Untuk membuat beberaka kateogri -> kategori tersebut untuk
        mengelompokan `publish_hour`

-   Fungsi `sapply()`

    -   Fungsi sapply bisa digunakan untuk menerabakan sebuah fungsi ke
        setiap baris pada dataframe kita

-   Fungsi `lapply()`

    -   Fungsi tersebut bisa digunakan untuk menerabkan sebuah fungsi ke
        setiap kolom pada dataframe kita

-   Fungso `match` & `unique`

    -   Kedua fungsi tersebut bisa digunakan untuk mengambil kemunculan
        unique yang pertama

-   Fungsi `plot()`

    -   Fungsi tersebut kita gunakan ketika kita ingin melaukan tahapan
        exploratory

        -   Tahapan eksploratory dilakukan ketika kita ingin melakukan
            analisis dengan cepat
        -   Tahapan eksploratory dilakukan ketika hasil visualisasinya
            tidak akan ditampilkan kepada orang lain
...
-   Untuk rentan waktu video agaar bisa diminati bisa diupload dari
...
-   Like ratio yang paling sedikit dimiliki oleh category Autos &
    vechicles
...
-   Kategori yang memiliki like ratio yang tinggi tapi dislikes rationya
    yang rendah adalah gaming
-   Jika dilihat dari garis trend, semakin tinggi like ratio maka
    semakin tinggi dislike ratio (cenderung lemah)
...
-   kategori gaming memiliki like ratio dan comment ratio yang paling
    besar disbanding 2 kategori lain
...
-   Jika kita lihat dari plot diatas, channel title yang paling sering
    trending memiliki category talkshow
...
-   Base plot
    ...
    -   Jika data yang digunakan hanyak 1 tipe numerik, maka hasil plot
        adalah hist

        -   Kita bisa menggunakan fungsi `hist()`

    -   Jika data yang digunakan hanyak 1 tipe katgorikal, maka hasil
        plot adalah bar

    -   Jika data yang digunakan hanyak 2 tipe numerik, maka hasil plot
        adalah scatter

    -   Jika data yang digunakan 1 tipe numerik & 1 tipe kategorikal,
        maka hasil plot adalah boxplot

-   Base plot kita gunakan untuk tahapan exploratory

-   `Library(ggplot2)` adalah library visuaslisasi untuk tahapan
    explanatory
...
```{r}
# Please Type Your Code Down Below
vids.agg <- aggregate.data.frame(x = list(kolom_1 = vids.unik$like_ratio,
                                          kolom_2 = vids.unik$dislike_ratio),
...
```{r}
# Please type your code down below
pivot_wider(data = vids.agg.long,
            names_from = name,
            values_from = value)
```
...
*frekuensi untuk setiap rentan waktu upload* pada *setiap kategori*
channel youtube.
...
vids.wc <- as.data.frame(table(vids.unik$publish_when,vids.unik$category_id))
vids.wc
```

```{r}
colnames(vids.wc) <- c("publish_when","category","freq")
vids.wc
```

```{r}
ggplot(data = vids.wc, mapping = aes(x= freq, y= reorder(category,freq))) +
...
```{r}
ggplot(data = vids.wc, aes(x = freq, y = reorder(category, freq)))+
...
-   Data Wrangling:
    ...
    -   `aggegate.data.frame()` -> fungsi ini bisa gunakan untuk
        melakukan agregasi untuk lebih dari satu kolom

        -   Paremter yang bisa digunaakn adalah `x`, `by` & `FUN`

    -   `pivot_longer` & `pivot_wider` -> untuk melakukan transformasi
        dari bentuk dataframe kita

        -   Parameter pivot_longer adalah `data` & `col`
        -   Parameter pivow_wider adalah `data`, `names_from`,
            `values_from`

-   Data Visualization:

    -   *Multivariate Plot* -> adalah sebuah plot untuk menampilkan
        tidak hanyak satu value numerik saja per category, melainkan
        kita bisa menampilkan lebih dari 1.

        -   

            Untuk menghasilkan multivariate plot kita bisa menambahkan parameter `position =`

            :   Value yang bisa dimasukan kedalam parameter position
                adalah `dodge` & `fill`

    -   *Facet Plot* -> hasil dari plotnya akan dipisahkan ke frame yang
        terpisah

        -   

            Untuk memisahkan ke frame yang berbeda kita bisa menggunakan fungsi `facet_wrap`

            :   Parameter yang bisa digunakan dari fungsi `facet_wrap`
                adalah `facets` & `scales`

    -   `scale_fill_manual` -> kita bisa menggunakan fungsi ini untuk
...
Additional reference for customization:
<https://ggplot2-book.org/polishing.html>

**DIVE DEEPER**

Business Case: Coba visualisasikan trending dari konten kategori
`Gaming`, `Entertainment`,dan `Music` dengan menggunakan line plot dari
tanggal *2018-01-01*.
...
Kita dapat membuat interactive map dengan bantuan package `leaflet`.
Leaflet sendiri adalah open-source JavaScript library untuk pembuatan
interactive maps.
...
Berikut adalah website dan referensi yang dapat mempermudah pemahaman
kita mengenai leaflet:

-   [Leaflet for R](https://rstudio.github.io/leaflet/)
-   [Creating Choropleth Map for Geocoding in
    R](https://algotech.netlify.com/blog/creating-choropleth-with-mapshaper-and-r/)

# Further Readings

Berikut adalah referensi-referensi lain dan website yang dapat
memudahkan kita untuk mengekplorasi seputar data visualisasi di R:

-   [ggplot2: Elegant Graphics for Data Analysis by Hadley
    Wickham](https://ggplot2-book.org/)

-   [Data-to-Viz](https://www.data-to-viz.com/): ragam tipe data dan
    plot yang cocok untuk data tersebut, dilengkapi contoh plot dan
    coding

-   [Algotech - Rplicate
    Series](https://algotech.netlify.app/tags/rplicate/)

-   [ggplot2 extensions](https://exts.ggplot2.tidyverse.org/):
    fitur-fitur tambahan yang bisa diintegrasi dengan ggplot2

-   [Eye Dropper Chrome
    extensions](https://chrome.google.com/webstore/detail/eye-dropper/hmdcmlfkchdmnmnmheododdhjedfccka):
    untuk memudahkan memilih warna costum dari internet

-   [HTML ColorCodes](https://htmlcolorcodes.com): untuk memilih warna
    costum dari internet
