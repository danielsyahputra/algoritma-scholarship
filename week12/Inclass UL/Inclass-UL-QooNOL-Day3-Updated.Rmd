...
Untuk lebih detail terkait image compression, silahkan kunjungi artikel saya: <https://medium.com/analytics-vidhya/image-compression-using-k-means-clustering-and-principal-component-analysis-in-python-893fd37e8e15>
...
-   `BOROUGH`, `BLOCK`, `LOT`: diganti jadi faktor
-   `ZIP.CODE`: faktor
-   `RESIDENTIAL.UNITS`, `COMMERCIAL.UNITS`, `TOTAL.UNITS`: tetap integer
-   `YEAR.BUILT`: faktor
-   `TAX.CLASS.AT.TIME.OF.SALE`: faktor

Perhatikan yang character:

-   `NEIGHBORHOOD`: faktor
-   `BUILDING.CLASS.CATEGORY`, `BUILDING.CLASS.AT.PRESENT`, `BUILDING.CLASS.AT.TIME.OF.SALE`: faktor
-   `TAX.CLASS.AT.PRESENT`: faktor
-   `ADDRESS`: tetap char
-   `APARTMENT.NUMBER`: tetap char karena ada abc nya
-   `LAND.SQUARE.FEET`, `GROSS.SQUARE.FEET`: numeric
-   `SALE.PRICE`: numeric
-   `SALE.DATE`: date

```{r warning=FALSE}
property_clean <- property %>% 
  select(-c(X, EASE.MENT)) %>% # untuk memilih kolom 
  mutate_at(vars(BOROUGH, BLOCK, LOT, ZIP.CODE,  TAX.CLASS.AT.TIME.OF.SALE), as.factor) %>% # melakukan perubahan terhadap beberapa kolom sekaligus
...
-   `complete.cases()` fungsi yang digunakan untuk mengambil baris yang kolomnya ada isinya semua
...
# data untuk PCA
ppt <- property_clean %>% 
  select_if(is.numeric) # kita hanya ingin mengambil kolom dengan kriteria tertentu
...
```{r}
pca <- prcomp(ppt, scale = TRUE) # parameter scale dari prcomp menggunakan z-score scaling
```
...
Kalau Bapak Ibu ingat untuk mendapatkan standar deviasi, Bapak Ibu menggunakan variance.

```{r}
pca$sdev #besaran informasinya 
```
...
bisa disimpulkan: 1 PC = 1 eigenvalue (variance, 1 nilai) + 1 eigenvector (formula/koefisien)
...
### menggabungkan kembali pca kepada data asal
...
property_clean %>% 
  select_if(is.factor) %>% 
  cbind(pc_keep) %>% # untuk menggabungkan kolom 
  head()
...
-   Dapat meningkatkan performa model dalam melakukan prediksi (tidak selalu)
-   Dapat mengatasi multicollinearity, karena nilai antar PC tidak saling berkorelasi (selalu)
...
💡 Slide: <https://docs.google.com/presentation/d/1iF_VjV5oTXJSkNwiWhyKm-4ugqlVdUZTFtRfrVS2c_o/edit#slide=id.gb7882dd136_0_1205>
...
Pertanyaan Ibu Dessy : memunculkan screeplot
...
-   Eigen vector = matrix rotasi untuk memproyeksikan data asli menjadi nilai PC (sumbu baru) (komposisi yang menyusun pc)
...
```{r}
# subset 100 data ppt
ppt_100 <- ppt %>% head(100)

# buat pca
ppt_100_pca <- prcomp(ppt_100, scale = T)
  
# visualisasikan dengan fungsi biplot
...
-   Titik-titik = index angka dari **observasi** (nomor baris). Semakin berdekatan maka karakteristiknya semakin mirip, sedangkan yang jauh dari gerombolan data dianggap sebagai outlier
...
-   PC1 merangkum variable: **selain Total Units dan Residential Units**
-   PC2 merangkum variable: **Total Units dan Residential Units**
...
-   `X`: objek PCA
-   `choice = "var"`: mau melihat **VAR**iable contribution
...
# variable contribution untuk PC1
fviz_contrib(X = ppt_100_pca, choice = "var", axes = 1)
```

```{r}
# variable contribution untuk PC2
fviz_contrib(X = ppt_100_pca, choice = "var", axes = 2)
```
...
cor(ppt_100$RESIDENTIAL.UNITS,ppt_100$TOTAL.UNITS)
```

Panah yang (hampir) tegak lurus adalah:

-   SALE.PRICE dan TOTAL.RESIDENT

```{r}
# opsional: untuk validasi
cor(ppt_100$SALE.PRICE, ppt_100$TOTAL.UNITS)
...
> Dari biplot di atas, tiga outlier terluar terletak pada observasi 96, 32, 51
...
-   index 96 -> ekstrim di `TOTAL.UNITS`
-   index 32 -> `TOTAL.UNITS` cukup besar nilainya, ekstrim di `LAND.SQUARE.FEET`
-   index 51 -> `SALE.PRICE` dan `GROSS.SQUARE.FEET` besar nilainya

```{r}
# outlier: 96, 32, 51
# non-outlier: 26 dan 27
ppt_100[c(96, 32, 51, 26, 27, 28),]
```

> Observasi 96 dan 32 merupakan outlier dan ternyata karena total.unitsnya sangat besar dibandingkan observasi yang non-outlier

```{r}
# index 82 -> bertolak belakang dengan panah TOTAL.UNITS, maka nilainya rendah
# index 96 -> searah dengan panah TOTAL.UNITS, maka nilainya tinggi
ppt_100[c(82, 96), ]
```
...
# buat pca
ppt_300_pca <- prcomp(ppt_300, scale = T)
  
# visualisasikan dengan fungsi biplot

biplot(ppt_300_pca, cex =0.6, scale =F)
...
-   Tiga observasi yang memiliki nilai residential dan total unit yang sangat tinggi: 110 114 115 (Bu Atika)
-   Tiga observasi yang memiliki nilai jual (SALE.PRICE) yang sangat tinggi: 118 116 103 (Ibu Shella)
...
1.  Variabel yang berkontribusi tinggi ke PC1: gross , sale , land
2.  Variabel yang berkontribusi tinggi ke PC2: residental, total
3.  Variabel yang berkontribusi rendah pada PC1 maupun PC2: year, commersial
...
fviz_contrib(X = ppt_300_pca , choice = "var", axes = 2)
```
...
-   land squre feet, gross square feet
-   residental unit , total unit

2.  Pasangan variabel yang tidak berkorelasi (tegak lurus):

-   land squre feet , total unit
...
1.  Perhatikan cluster di mana Arizona, Colorado, Illinois, dan Texas berada. Negara bagian mana lagi yang ada dalam cluster tersebut? new york, negara bagian pada cluster ini, sangat banyak kasus rape

2.  Di antara Ohio, Louisiana, North Dakota, dan Colorado, manakah yang memiliki:

-   Tingkat `Murder` tertinggi: Lousiana
-   Tingkat `Assault` tertinggi: Lousiana
...
-   grdCto A, verified, not paid

```{r}
loan_clean <- loan %>% 
  select(-c(verification_status, grade)) %>% 
...
quantivar <- c(3,4,5,6,9,10,11,14)
...
loan_pca <- PCA(X = loan_clean, 
                scale.unit = TRUE,
                quali.sup = qualivar,
                graph = FALSE,
                ncp = 8)

loan_pca$eig # sama dengan summary(pca) di prcompt
```
...
plot.PCA(x = loan_pca , 
         choix = "ind",
         invisible = "quali",
         select = "contrib 5", # 5 outlier terluar
         habillage = "purpose") 
```

Lima outlier terluar: 512 , 749, 368,1146,1228
...
-   PC1 paling banyak merangkum variable: annual_inc

-   PC2 merangkum variable: int_rate

-   Variabel yang saling berkorelasi tinggi positif:

    -   revol bal installment
    -   dti int_rate
    -   annual_inc log inc
...
```{r}
fviz_contrib(X = loan_pca, choice = "var", axes = 2)
```

⚠️ Besar kontribusi tidak dilihat dari tanda negatif/positif pada korelasi, melainkan nilai absolutnya
...
loan_reconst <- reconst(loan_pca, ncp = 5 ) # karena cuma ingin ambil 5 pc
...
Clustering adalah pengelompokan data berdasarkan karakteristiknya. Clustering bertujuan untuk menghasilkan cluster/kelompok dimana:
...
Apakah ada kolom yang bisa dibuang?

```{r}

whi_clean <- whisky %>% 
  select(-c(RowID, Distillery, Postcode, Longitude, Latitude))
```

Cek kembali data whisky
...
Jawaban : tidak perlu scaling karena data dasar sudah punya range tetap yaitu 0 - 4
...
whi_km <- kmeans(x= whi_clean , centers = 3)
whi_km
```
...
RNGkind(sample.kind = "Rounding")
set.seed(100)
whi_km_80 <- kmeans(x = whi_clean, centers = 80)
```
...
Jawaban :k terlalu besar, setiap cluster isinya sedikit jadi tidak representatif, meskipun metrics evaluasinya bagus.
...
whi_km_opt <- kmeans(x= whi_clean, centers = 5)
...
whisky$cluster <- whi_km_opt$cluster
...
-   Cluster 1: Dominan Body and Fruity
-   Cluster 2: Dominan Smoky and Body, Tabacco
-   Cluster 3: Dominan Swetness and Floral
-   Cluster 4: Dominan Swetness and Smoky
-   Cluster 5: Dominan Medicinal and Floral
...
**Kesimpulan**: Laphroig ada di cluster 2, sehingga kita bisa merekomendasikan whiskey seperti Ardberg, dsb.
...
whi_clean$cluster <- whi_km_opt$cluster
whisky_pca <- PCA(X = whi_clean, quali.sup = 13, scale.unit = F, graph = F)
fviz_pca_biplot(whisky_pca, habillage = 13, addEllipses = T)
```
...
fviz_cluster(object = whi_km_opt, data = whi_clean)
...
-   [Algotech : INTRODUCTION TO HIERARCHICAL CLUSTERING](https://algotech.netlify.app/blog/introduction-to-hierarchical-clustering/)
